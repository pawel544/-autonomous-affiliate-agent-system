...
def get_opinion_type(id:int) ->str:
    BASE_DIR = Path(__file__).resolve().parent.parent.parent.parent
    DB_FILE = BASE_DIR / 'data' / 'base.db'
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("SELECT final_score FROM  program_affiliate WHERE id =?" ,(id,))
